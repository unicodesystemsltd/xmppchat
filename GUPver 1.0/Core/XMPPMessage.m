...
- (BOOL)isChatMessage
{
	return [[[self attributeForName:@"type"] stringValue] isEqualToString:@"chat"];//||[[[self attributeForName:@"type"] stringValue] isEqualToString:@"image"]||[[[self attributeForName:@"type"] stringValue] isEqualToString:@"audio"]||[[[self attributeForName:@"type"] stringValue] isEqualToString:@"vcard"];
}
-(BOOL)isMessageResending
{
    if ([self isChatMessage])
    {
        return  [[[self attributeForName:@"isResend"]stringValue ] boolValue]==true?YES:NO;
    }
    return NO;
...
- (BOOL)isAcknoledgment
{
	return [[[self attributeForName:@"type"] stringValue] isEqualToString:@"chat"];
}
- (BOOL)isChatMessageAsNotification
{
	if ([self isChatMessage])
	{
		return [self isNotification];
	}
...
{
	return ([self elementForName:@"gup"] != nil);
}
- (BOOL)isNotification
{
	if ( [self elementForName:@"gup"] != nil)
        {  NSXMLElement *gup=[self elementForName:@"gup"];
            if ( [[[gup elementForName:@"grpUpdate"] stringValue] boolValue]||[[[gup elementForName:@"contactUpdate"] stringValue] boolValue]||[[[gup elementForName:@"gupNotification"] stringValue] boolValue]||[[[gup elementForName:@"contactDelete"] stringValue] boolValue]||[[[gup elementForName:@"is_notify"] stringValue] boolValue])
                return true;
        }
    return false;
}
